...
echo "=== 应用 Shortcut FE 内核 6.12 补丁 ==="

# 创建补丁目录
mkdir -p target/linux/generic/hack-6.12

# 创建补丁文件
cat > target/linux/generic/hack-6.12/953-net-patch-linux-kernel-to-support-shortcut-fe.patch << 'EOF'
...
echo "✅ Shortcut FE 内核补丁已创建"

# 检查是否需要应用补丁
if [ -f "target/linux/generic/config-6.12" ]; then
    echo "为内核 6.12 启用 Shortcut FE 配置..."
    if ! grep -q "CONFIG_SHORTCUT_FE" target/linux/generic/config-6.12; then
        echo "CONFIG_SHORTCUT_FE=y" >> target/linux/generic/config-6.12
        echo "✅ 已启用 CONFIG_SHORTCUT_FE"
    else
        echo "⚠️ CONFIG_SHORTCUT_FE 已存在"
    fi
fi
